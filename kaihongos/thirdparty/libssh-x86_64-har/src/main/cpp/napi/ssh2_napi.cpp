...
napi_value SSH2Napi::GetServerBanner(napi_env env, napi_callback_info info) {
    size_t argc = PARAM_COUNT_2;
    napi_value args[PARAM_COUNT_2] = {nullptr};
    napi_value thisVal = nullptr;
    napi_get_cb_info(env, info, &argc, args, &thisVal, nullptr);
    ...
    napi_create_promise(env, &deferred, &promise);
    napi_value resourceName;
    napi_create_string_latin1(env, "getServerBanner", NAPI_AUTO_LENGTH, &resourceName);
    SSH2Client *ss2Client = new SSH2Client();
    SSH2Napi *ssh2Napi = nullptr;
    napi_unwrap(env, thisVal, (void **)&ssh2Napi);
    if (!ssh2Napi) {
    ...
    napi_create_async_work(
        env, nullptr, resourceName,
        [](napi_env env, void *data) {
            SSH2ClientContext *ctx = (SSH2ClientContext *)data;
            ctx->buffer = ctx->ssh2Client->FetchServerBanner(ctx->ip, ctx->port);
        },
        [](napi_env env, napi_status status, void *data) {
            SSH2ClientContext *ctx = (SSH2ClientContext *)data;
            napi_value result = NapiUtil::SetNapiCallString(ctx->env, ctx->buffer);
