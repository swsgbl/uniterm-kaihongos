...
public:
    ...
    void SetServerBanner(const char *banner);

    std::string GetServerBanner();

    // standalone short-lived connection: connect ip:port, read banner, disconnect. empty string on failure.
    std::string FetchServerBanner(std::string ip, std::string port);

public:
    ssh_session _ssh_session;
    ssh_channel _ssh_channel;
    bool isStop{false};
    std::string _server_banner{""};
