...
int SSH2Client::Start(std::string ip, std::string port, std::string privateKeyPath, std::string user,
    ...
    const char *banner = ssh_get_serverbanner(ssh_session);
    if (banner != NULL) {
        this->_server_banner = std::string(banner);
        LOGI("SSH server banner: %s", banner);
    } else {
        this->_server_banner = std::string("");
        LOGW("SSH server banner: unavailable");
    }
    ssh_key server_key;
...
void SSH2Client::SetServerBanner(const char *banner) {
    this->_server_banner = std::string(banner != NULL ? banner : "");
}

std::string SSH2Client::GetServerBanner() {
    return this->_server_banner;
}

std::string SSH2Client::FetchServerBanner(std::string ip, std::string port) {
    std::string result = "";
    ssh_session session = ssh_new();
    if (session == NULL) {
        LOGE("FetchServerBanner: ssh_new failed");
        return result;
    }
    ssh_options_set(session, SSH_OPTIONS_HOST, ip.c_str());
    int _port = NapiUtil::StringToInt(port);
    ssh_options_set(session, SSH_OPTIONS_PORT, &_port);
    long timeout_sec = 5;
    ssh_options_set(session, SSH_OPTIONS_TIMEOUT, &timeout_sec);
    /* OHOS app sandbox has no passwd entry for the app uid and no $HOME, so
     * ssh_options_apply() would fail in ssh_get_local_username()/~ expansion
     * ("Couldn't apply options"). Set both explicitly. */
    ssh_options_set(session, SSH_OPTIONS_USER, "uniterm");
    ssh_options_set(session, SSH_OPTIONS_SSH_DIR, "/data/storage/el2/base/temp");
    int rc = ssh_connect(session);
    if (rc != SSH_OK) {
        LOGE("FetchServerBanner: connect %s:%s failed: %s", ip.c_str(), port.c_str(), ssh_get_error(session));
        ssh_free(session);
        return result;
    }
    const char *banner = ssh_get_serverbanner(session);
    if (banner != NULL) {
        result = std::string(banner);
        LOGI("FetchServerBanner: %s", banner);
    } else {
        LOGW("FetchServerBanner: banner is empty");
    }
